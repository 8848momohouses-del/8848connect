...
class MarketingFeeStatement(models.Model):
    ...
    marketing_fee_percentage = fields.Float(string='Marketing Fee (%)', related='franchise_id.marketing_fee_percentage', readonly=True)
    marketing_fee_amount = fields.Monetary(string='Marketing Fee Amount', compute='_compute_fee_amount', store=True, tracking=True)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('8848.marketing.fee.statement') or _('New')
        return super().create(vals_list)

    @api.depends('total_sales', 'marketing_fee_percentage')
    def _compute_fee_amount(self):
        for record in self:
            record.marketing_fee_amount = (record.total_sales * record.marketing_fee_percentage) / 100.0
    ...
    def action_generate_invoice(self):
        ...
        invoice_vals = {
            ...
            'invoice_line_ids': [(0, 0, {
                'name': _('Marketing Fee for %s to %s') % (self.date_start, self.date_end),
                'quantity': 1,
                'price_unit': self.marketing_fee_amount,
            })],
        }
